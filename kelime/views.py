...
def question(request):
    ...
    while control:
        ...
        count = KelimeBilgi.objects.filter(
            word=kelime, user=request.user).count()
        if(count == 0):
            control = False
        ...
        oran=(kayitliVeri*100)/kelimeSayisi
        oran=int(oran)
        print(oran)
        print(kayitliVeri)
        print(kelimeSayisi)
...
def statistics(request):
    count = KelimeBilgi.objects.filter(user=request.user,date__year='2019', date__month='05').count()
    print(count)
    return render(request, "statistics.html",{"count": count})
